...
class AiTutorViewModelTest {
    ...
    fun setup() {
        ...
        kotlinx.coroutines.runBlocking {
            whenever(llmRepository.sendMessage(anyOrNull(), anyOrNull(), anyOrNull(), anyOrNull(), anyOrNull(), anyOrNull())).thenReturn(flowOf("Mocked response"))
        }
    ...
    @Test
    fun `onInputChanged updates text`() = runTest(testDispatcher) {
        viewModel.onInputChanged("Hello AI")
        assertEquals("Hello AI", viewModel.uiState.value.inputText)
    }

    @Test
    fun `clearErrorMessage removes error`() = runTest(testDispatcher) {
    ...
    @Test
    fun `setApiSettingsVisible updates state correctly`() = runTest(testDispatcher) {
    ...
    @Test
    fun `onSuggestionClicked updates input text and sends message`() = runTest(testDispatcher) {
    ...
    @Test
    fun `startNewChat clears input and messages`() = runTest(testDispatcher) {
