...
class VideoDownloadViewModelTest {
    ...
    @Test
    fun `addDownloadTask creates task and starts download`() = runTest(testDispatcher) {
        val url = "http://example.com/video"
        `when`(downloader.downloadVideo(org.mockito.kotlin.any(), org.mockito.kotlin.any())).thenReturn(Result.success("path/to/video.mp4"))
        `when`(summarizeVideoUseCase.invoke(org.mockito.kotlin.any(), org.mockito.kotlin.any(), org.mockito.kotlin.any(), org.mockito.kotlin.any(), org.mockito.kotlin.any())).thenReturn(Result.success("Summary text"))
    ...
    @Test
    fun `cancelDownload sets task status to cancelled`() = runTest(testDispatcher) {
        viewModel.addDownloadTask("http://example.com/video")
        ...
        assertEquals(com.example.summarizer.video_summarizer.services.DownloadStatus.CANCELLED, updatedTask?.progress?.status)
    }

    @Test
    fun `handleLocalVideo adds task and starts summary`() = runTest(testDispatcher) {
        val uri = org.mockito.kotlin.mock<android.net.Uri>()
        val mockFile = java.io.File("local/path/video.mp4")
        `when`(processLocalVideoUseCase(org.mockito.kotlin.any(), org.mockito.kotlin.any())).thenReturn(Result.success(Pair("video.mp4", mockFile)))
        `when`(summarizeVideoUseCase.invoke(org.mockito.kotlin.any(), org.mockito.kotlin.any(), org.mockito.kotlin.any(), org.mockito.kotlin.any(), org.mockito.kotlin.any())).thenReturn(Result.success("Summary text"))
