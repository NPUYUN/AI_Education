...
@Singleton
class DialogueSummaryRepository @Inject constructor(
    ...
    suspend fun summarizeDialogue(
        ...
        try {
            ...
            val request = ChatRequest(
                model = modelName,
                messages = listOf(
                    ChatMessage(role = "system", content = com.example.common.config.AppConstants.DIALOGUE_SUMMARY_SYSTEM_PROMPT),
                    ChatMessage(
                        role = "user",
                        content = "以下是对话记录：\n\n$dialogueText"
                    )
