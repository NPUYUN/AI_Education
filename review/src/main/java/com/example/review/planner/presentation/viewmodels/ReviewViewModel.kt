    ...
    constructor(
        ...
        fun generateReviewPlan() {
            ...
            if (!networkMonitor.isConnected.value) {
                viewModelScope.launch { _errorEvents.send("????????????±Á??????????????????") }
                return
            }
            ...
            viewModelScope.launch {
                try {
                    ...
                    if (result.isSuccess) {
                        ...
                        val errorMsg = exception?.toUserFriendlyMessage() ?: "¦Ä?????"
        ...
        fun addMockErrorRecord() {
            viewModelScope.launch {
                val record =
                    ErrorBookEntity(
                        subject = "???",
                        questionContent = "???? f(x) = x^2 - 4x + 3 ????§³???",
                        errorReason = "??????????????????",
                        correctSolution = "f(x) = (x-2)^2 - 1????????§³?? -1??",
                    )
